...
| `triggerSources` | `game.ts:queueTriggers` + `characteristics.ts:staticSources` | extra objects to scan beyond `allPermanents` (emblems, command-zone statics); they are scanned for triggers even when nothing on the battlefield carries the trigger, and their static abilities are folded into `Mods` like a permanent's (CR 114.2) |
...
| `keywordHooks.attackFixup` | `game.ts:combatFrom` + `simulateCombat` | rewrite the FINISHED attack declaration (a `Set` of object ids) before anything is tapped, before the attack event and before an `attacks` trigger is queued — the restrictions only a whole declaration can judge ("~ can't attack alone", CR 506.4) |
...
| `replacements.damage` | `game.ts:dealDamage` / `dealDamageToPlayer`, after protection and **after** the core's own prevention shields (CR 616.1e: the affected player chooses the order and any order is legal; the core applies its shield first on their behalf — the order that never deals them more damage) | return the new amount |
| `replacements.preventable` | `game.ts:dealDamage` / `dealDamageToPlayer`, guarding the core's own shields | CR 615.12: `false` switches the core's own shields (`eotFlags.preventDamage`, the Fog flag) off for this one event without consuming them |
...
| `controlUntilEot` | `game.ts:applyEffect` `gain-control` with `duration: 'eot'` | a family that keeps control-change effects in timestamp order (CR 613.7) takes the core's until-end-of-turn theft over so it interleaves with its own durations; return `true` when recorded (the core then writes no `controlUntilEot`). A takeover fold: consulted newest-registered first, the first `true` wins |
| `transform` | `game.ts:applyEffect` `transform-self` (not its exile-and-return form) | perform or refuse the face flip yourself (daybound / nightbound, CR 702.145b; the `transforms` event at the instant of the flip, CR 603.2); return `true` when handled either way (a takeover fold, newest-registered first) |
