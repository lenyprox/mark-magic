...
export function Dialog({ open, onClose, title, children, footer, width, plain, className, labelledBy, closeLabel = 'Close' }: DialogProps) {
  ...
  return (
    <dialog ref={ref} className={clsx(styles.dialog, className)} style={width ? ({ '--dialog-w': `${width}px` } as CSSProperties) : undefined} aria-labelledby={labelledBy}
      onClick={(e) => { if (e.target === ref.current) onClose(); }}>
      <div className={styles.panel}>
        <IconButton label={closeLabel} className={styles.closeBtn} onClick={onClose}><X /></IconButton>
        {title && <div className={styles.head}><div className={styles.title}>{title}</div></div>}
