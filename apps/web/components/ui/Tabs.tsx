...
export interface TabsProps<T extends string> { value: T; onChange: (v: T) => void; items: TabItem<T>[]; label: string; className?: string }
...
export function Tabs<T extends string>({ value, onChange, items, label, className }: TabsProps<T>) {
  ...
  return (
    <div role="tablist" aria-label={label} className={clsx(styles.tabs, className)} onKeyDown={onKey}>
      {items.map(t => (
        <button key={t.value} type="button" role="tab" id={`${id}-${t.value}`} aria-selected={t.value === value} aria-controls={`${id}-panel-${t.value}`} tabIndex={t.value === value ? 0 : -1} className={styles.tab} onClick={() => onChange(t.value)}>
