...
import { plainCopy } from './ops/ext.js';
...
function serObject(o: GameObject): SerializedObject {
  ...
  const out = { ...(rest as Omit<GameObject, 'def' | 'activatedThisTurn' | 'triggeredThisTurn'>), defKey: defKey(def), activatedThisTurn: [...activatedThisTurn], ...(triggeredThisTurn ? { triggeredThisTurn: [...triggeredThisTurn] } : {}) };
  if (o.ext) out.ext = plainCopy(o.ext);   // family state is JSON-plain by contract (src/engine/ops/ext.ts)
  return out;
}

export function serializeState(s: GameState): SerializedState {
  const players = s.players.map(p => ({ ...p, ...(p.ext ? { ext: plainCopy(p.ext) } : {}), library: p.library.map(serObject), hand: p.hand.map(serObject), graveyard: p.graveyard.map(serObject), exile: p.exile.map(serObject), battlefield: p.battlefield.map(serObject), command: (p.command ?? []).map(serObject) }));
  ...
  return { ...(rest as Omit<GameState, 'players' | 'stack' | 'version'>), ...(s.ext ? { ext: plainCopy(s.ext) } : {}), version: 1, stateVersion: stateVersion as number, players, stack, log: [...s.log] };
}
...
function deObject(o: SerializedObject, defs: Map<string, CardDef>): GameObject {
  ...
  const out = { ...(rest as Omit<GameObject, 'def' | 'activatedThisTurn' | 'triggeredThisTurn'>), def, activatedThisTurn: new Set(activatedThisTurn), ...(triggeredThisTurn ? { triggeredThisTurn: new Set(triggeredThisTurn) } : {}) } as GameObject;
  if (o.ext) out.ext = plainCopy(o.ext);
  return out;
}
...
export function deserializeState(ser: SerializedState, defs: Map<string, CardDef>): GameState {
  ...
  const players = ser.players.map(p => ({ ...p, ...(p.ext ? { ext: plainCopy(p.ext) } : {}), library: p.library.map(de), hand: p.hand.map(de), graveyard: p.graveyard.map(de), exile: p.exile.map(de), battlefield: p.battlefield.map(de), command: (p.command ?? []).map(de), commanders: p.commanders ?? [], commanderCasts: p.commanderCasts ?? {}, commanderDamage: p.commanderDamage ?? {} })) as Player[];
  ...
  if (ser.ext) out.ext = plainCopy(ser.ext as Record<string, unknown>);
