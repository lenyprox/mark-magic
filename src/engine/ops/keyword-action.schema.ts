...
/**
 * The shape a family's `<family>.schema.ts` exports. `src/engine/ops/types.ts` has no `FamilySchema` type yet (the
 * schema composer is a separate slice), so it is declared here with exactly the shape the contract names and the
 * composer adopts it as-is: lists of strict variants keyed by discriminator, plus the string-literal vocabularies
 * (`amounts`, `targetKinds`) and the cost-part field map.
 */
