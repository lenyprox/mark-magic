...
//   (c) the generated barrels match what scripts/gen-registry.mjs produces (never hand-edit `_registry.ts` or
//       src/cards/_schemas.ts).
import { test } from 'node:test';
import assert from 'node:assert/strict';
import fs from 'node:fs';
import path from 'node:path';
import { familyFiles, familySchemaFiles, OUT_FILE, render, renderSchemas, SCHEMAS_OUT_FILE, SHARED } from '../scripts/gen-registry.mjs';
...
test('src/cards/_schemas.ts is what gen:registry produces (run `npm run gen:registry`)', () => {
  const onDisk = fs.readFileSync(SCHEMAS_OUT_FILE, 'utf8');
  assert.equal(onDisk, renderSchemas(), 'generated schema barrel is stale — run `npm run gen:registry` and commit the result');
  assert.ok(onDisk.startsWith('// GENERATED by scripts/gen-registry.mjs — do not edit'), 'the barrel must carry the generated header');
  assert.equal(onDisk.includes('\r'), false, 'the generated barrel must be written with LF endings');
  assert.equal(renderSchemas(), renderSchemas(), 'the generator is deterministic');
  // every `<family>.schema.ts` is listed, underscore-prefixed files are not, and the listing is name-sorted
  const listed = familySchemaFiles();
  const all = fs.readdirSync(OPS).filter(f => f.endsWith('.schema.ts'));
  assert.deepEqual(listed, all.filter(f => !f.startsWith('_')).sort());
  for (const f of listed) assert.ok(onDisk.includes(`'../engine/ops/${f.replace(/\.ts$/, '.js')}'`), `${f} is not imported by the schema barrel`);
  // and the generator itself skips underscore files by name, whatever the directory holds
  assert.deepEqual(familySchemaFiles(path.join(OPS)), listed);
});
