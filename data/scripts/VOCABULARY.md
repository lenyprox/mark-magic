...
From `src/engine/ops/combat-restr.schema.ts`. These are accepted by the checker exactly like the core vocabulary above.

#### Effects (`Effect.op`)

| op | fields (`?` = optional) |
|---|---|
| `blocks-if-able` | `target`, `duration` |
| `cant-block-source` | `target`, `duration` |
| `extra-combat` | — |
| `lure` | `target`, `duration` |
| `restrict-blocking` | `whose`, `filter?`, `duration` |

#### Static effects (`StaticEffect.kind`)

| kind | fields (`?` = optional) |
|---|---|
| `cant-act-alone` | `scope`, `filter?`, `side?`, `attack?`, `block?` |
| `cant-be-blocked-by` | `scope`, `filter?`, `side?`, `by` |
| `cant-be-blocked-except-by` | `scope`, `filter?`, `side?`, `by?`, `least?`, `power?` |
| `cant-block-creatures` | `scope`, `filter?`, `side?`, `what?`, `only?`, `power?` |
| `damage-as-though-unblocked` | `scope`, `filter?`, `side?` |
| `must-be-blocked` | `scope`, `filter?`, `side?`, `all?` |
