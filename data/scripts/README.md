...
| 4 | **lint** (`src/cards/lint.ts`) | counter names, `Filter.subtypes` against the generated subtype vocabulary, keywords / target kinds / amount counts / condition kinds against the schema's own enums, every `covers` / `ignore` line, triggers the card can never raise, `aiHints.role`, a `choose-objects` standing in for a printed "target" (targets are chosen when the ability is put on the stack, CR 115.1 / 601.2c — declare a `TargetSpec`) | an unknown name, a trigger that can never fire, or a resolution-time choice where the line prints "target" |
